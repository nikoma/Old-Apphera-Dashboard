...
class BingApi
  ...
  def perform(org_id)
    ...
    answer['d']['results'].each do |r|
      p r['Url']
      OrganizationLink.find_or_create_by_url!(:organization_id => @org_id, url: r['Url'], bing_id: r['ID'], bing_display_url: r['DisplayUrl'], description: r['Description'], title: r['Title']) rescue nil
